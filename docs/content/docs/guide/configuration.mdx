...
| `--yes` / `--execute` | Execute writes without staying in preview |
...
`peer config unset` removes a single key; `peer config reset` clears all stored values.
...
peer deposit create --params-file deposit-config.json --amount 250
```

This is useful for SDK-level parameters that don't have a dedicated CLI flag.
