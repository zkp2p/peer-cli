...
peer indexer query --query '{ __schema { queryType { fields { name } } } }'
```
...
These are deposit-adjacent OrchestratorV3 hooks and should be treated as governance/operator controls rather than day-to-day user actions. SDK 0.11's V2-only whitelist-hook helpers are not exposed; use its `accessPolicy` API for current whitelist policy management.
